...
trait ClassLikeSupport: 
  ...
  extension (c: ClassDef):
      def nonprivateMembers = c.body.filterNot(_.symbol.isHiddenByVisibility)
      ...
      def getGivenFields: List[DProperty] = {
        val valDefs = nonprivateMembers.collect { 
      ...
      def getConstructors: List[Symbol] = nonprivateMembers.collect {
          case d: DefDef if d.name == "<init>" && c.constructor.symbol != d.symbol => d.symbol
      }.toList

      def getNestedClasslikes: List[DClasslike] = nonprivateMembers.collect {
      ...
      def getTypeDefs: List[TypeDef] = nonprivateMembers.collect { 
        case td: TypeDef if !td.symbol.flags.is(Flags.Synthetic) && (!td.symbol.flags.is(Flags.Case) || !td.symbol.flags.is(Flags.Enum))
          => td 
      }

      def getValDefs: List[ValDef] = nonprivateMembers.collect { 
  ...
  def parseEnum(classDef: reflect.ClassDef)(using ctx: Context): DClass = 
    ...
    val enumVals = companion.nonprivateMembers.collect {
    ...
    val enumTypes = companion.nonprivateMembers.collect {
    ...
    val enumNested = companion.nonprivateMembers.collect {
